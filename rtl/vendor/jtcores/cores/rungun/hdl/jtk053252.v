...
always @(posedge clk) if(pxl_cen) begin
    // Keep one producer for the CCU output. Register reads come from the
    // generated MMR; other addresses expose the live counter readback.
    dout   <= (cs && rnw) ? mmr_dout :
              {vcnt[7:1],addr[0] ? vcnt[0] : vcnt[8]};
    hld    <= hld_mx;
    vld    <= vld_mx;
    hldi_l <= hldi;
    vldi_l <= vldi;
